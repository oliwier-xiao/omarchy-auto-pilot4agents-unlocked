...
lib_case nudge '
      ...
      eq(Model.parseTyped("17.09 1830", now), { ms: at(2026, 9, 17, 18, 30), rolled: false, dated: true })
      eq(Model.parseTyped("17/9 18:30", now), { ms: at(2026, 9, 17, 18, 30), rolled: false, dated: true })
      eq(Model.parseTyped("17.09.", now, at(2026, 9, 14, 23, 0)), { ms: at(2026, 9, 17, 23, 0), rolled: false, dated: true })
      eq(Model.parseTyped("17.09", now), { ms: at(2026, 9, 17, 7, 22), rolled: false, dated: true })
      eq(Model.parseTyped("2026-09-20 0700", now), { ms: at(2026, 9, 20, 7, 0), rolled: false, dated: true })
      eq(Model.parseTyped("14.09.2026  9:05", now), { ms: at(2026, 9, 14, 9, 5), rolled: false, dated: true })
      eq(Model.parseTyped("10.09", now), { ms: at(2027, 9, 10, 7, 22), rolled: true, dated: true })
      eq(Model.parseTyped("31.09", now), null)
      eq(Model.parseTyped("17.13", now), null)
      eq(Model.parseTyped("17.09 2460", now), null)
      eq(Model.parseTyped("17-09", now), null)
      eq(Model.onDay(at(2026, 9, 18, 0, 0), at(2026, 9, 14, 18, 45), now), at(2026, 9, 18, 18, 45))
      eq(Model.onDay(at(2026, 9, 18, 0, 0), NaN, now), at(2026, 9, 18, 7, 22))
      var strip = Model.dayStrip(now, 9)
      eq([strip.length, strip[0].key, strip[0].weekday, strip[0].day, strip[8].key, strip[8].weekday],
         [9, "2026-09-14", "Mon", 14, "2026-09-22", "Tue"])
      eq(Model.dayStrip(at(2026, 9, 29, 12, 0), 3).map(function (d) { return d.day + " " + d.month }), ["29 Sep", "30 Sep", "1 Oct"])
