...
Item {
  ...
  function step() {
    ...
    if (s === 3) {
      ...
      root.check(root.when.stops.slice(3, 7).join(",") === "now,minus60,minus30,plus30", "Now, then the steps back, then forward")
      root.press(Qt.Key_Right); root.press(Qt.Key_Right); root.press(Qt.Key_Right); root.press(Qt.Key_Right)
      root.check(root.when.cursorStop === "plus60", "cursor on +1h")
      root.press(Qt.Key_Return)
      root.press(Qt.Key_Return)
      t = root.compose.trigger
      root.check(t.kind === "in" && t.delaySec === 7200, "+1h twice is a 2 h delay")
      // Stepping back after overshooting: -30m, then -1h twice lands on Now, not a minute out.
      root.press(Qt.Key_Left); root.press(Qt.Key_Left)
      root.check(root.when.cursorStop === "minus30", "cursor on -30m")
      root.press(Qt.Key_Return)
      root.check(root.compose.trigger.kind === "in" && root.compose.trigger.delaySec === 5400, "-30m: 1 h 30 m")
      root.press(Qt.Key_Left)
      root.press(Qt.Key_Return)
      root.check(root.compose.trigger.kind === "in" && root.compose.trigger.delaySec === 1800, "-1h: 30 m")
      root.press(Qt.Key_Return)
      root.check(root.compose.trigger.kind === "now", "-1h from 30 m goes back to Now")
      root.press(Qt.Key_Return)
      root.check(root.compose.trigger.kind === "now", "-1h at Now stays Now")
      root.press(Qt.Key_Right); root.press(Qt.Key_Right); root.press(Qt.Key_Right)
      root.press(Qt.Key_Return)
      root.press(Qt.Key_Return)
      root.check(root.compose.trigger.kind === "in" && root.compose.trigger.delaySec === 7200, "back to a 2 h delay")
      root.press(Qt.Key_R)
