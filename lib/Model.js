...
function clockParts(text) {
  var m = /^([0-9]{1,2}):([0-9]{2})$/.exec(text) || /^([0-9]{1,2})([0-9]{2})$/.exec(text)
  if (!m) return null
  var h = parseInt(m[1], 10), mi = parseInt(m[2], 10)
  return h > 23 || mi > 59 ? null : { h: h, mi: mi }
}

// Typed times: "1405", "905", "14:05". A time already passed today means tomorrow.
// Typed dates, day first, with an optional time after a space: "17.09", "17/9 1830",
// "17.09.2026 18:30", or year first "2026-09-17 18:30". They answer { ms, rolled,
// dated: true }. A date without a time keeps the clock of `baseMs` (else now's); a date
// without a year is this year's, and rolled into next year when that day has passed.
function parseTyped(text, nowMs, baseMs) {
  var t = String(text === undefined || text === null ? "" : text).trim().replace(/\s+/g, " ")
  if (!isFinite(Number(nowMs))) return null
  var clock = clockParts(t)
  if (clock) return atClock(clock.h, clock.mi, nowMs)
  var dm = /^([0-9]{1,2})[.\/]([0-9]{1,2})(?:[.\/]([0-9]{4}))?\.?(?: (.+))?$/.exec(t)
  var ymd = dm ? null : /^([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})(?: (.+))?$/.exec(t)
  if (!dm && !ymd) return null
  var day = parseInt(dm ? dm[1] : ymd[3], 10)
  var month = parseInt(dm ? dm[2] : ymd[2], 10)
  var yearText = dm ? dm[3] : ymd[1]
  var timeText = dm ? dm[4] : ymd[4]
  if (timeText !== undefined) {
    clock = clockParts(timeText)
    if (!clock) return null
  } else {
    var base = validDate(isFinite(Number(baseMs)) ? Number(baseMs) : Number(nowMs))
    clock = { h: base.getHours(), mi: base.getMinutes() }
  }
  var year = yearText !== undefined ? parseInt(yearText, 10) : new Date(Number(nowMs)).getFullYear()
  var d = new Date(year, month - 1, day, clock.h, clock.mi, 0, 0)
  if (d.getMonth() !== month - 1 || d.getDate() !== day) return null
  var rolled = false
  if (yearText === undefined && localMidnight(d.getTime()) < localMidnight(nowMs)) {
    d = new Date(year + 1, month - 1, day, clock.h, clock.mi, 0, 0)
    if (d.getMonth() !== month - 1) return null
    rolled = true
  }
  return { ms: d.getTime(), rolled: rolled, dated: true }
}

// The days a job can be set on, today first: local midnight, its key and its labels.
function dayStrip(nowMs, count) {
  var start = localMidnight(nowMs)
  if (!isFinite(start)) return []
  var out = []
  for (var i = 0; i < Math.max(0, Math.round(Number(count)) || 0); i++) {
    var d = new Date(start)
    d.setDate(d.getDate() + i)
    out.push({ ms: d.getTime(), key: dayKey(d.getTime()), weekday: WEEKDAYS[d.getDay()], day: d.getDate(),
               month: MONTHS[d.getMonth()], offset: i })
  }
  return out
}

// The clock time of `baseMs` (else of `nowMs`) on the local day of `dayMs`.
function onDay(dayMs, baseMs, nowMs) {
  var day = validDate(dayMs)
  var base = validDate(isFinite(Number(baseMs)) ? Number(baseMs) : Number(nowMs))
  if (!day || !base) return NaN
  return new Date(day.getFullYear(), day.getMonth(), day.getDate(), base.getHours(), base.getMinutes(), 0, 0).getTime()
}
